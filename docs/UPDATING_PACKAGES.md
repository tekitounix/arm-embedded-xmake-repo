...
| ユースケース | 推奨パターン | 理由 |
|-------------|-------------|------|
| 単一バッファ処理 | `for (auto& s : span)` | 最速（index の 2.3 倍） |
| 入力→出力変換 | `std::transform` | 明確な意図、同等性能 |
| ステレオ同期処理 | `for (i...)` index loop | zip より可読性良く同等性能 |
| インデックス必要時 | `for (i...)` index loop | enumerate 未対応のため |

**注意**: `std::views::zip` は使用可能だが、index ループと比較して性能メリットがなく、可読性も劣るため非推奨。
