...
task("flash")
    ...
    set_menu {
        ...
        options = {
            ...
            {'a', "address", "kv", nil, "Specify base address for flashing (e.g., 0x08000000)"},
...
$ pyocd list --targets
        ...
        -- Check if pyocd is available (via shared tool registry)
        import("utils.tool_registry", {alias = "tool_registry"})
        local pyocd = tool_registry.find_pyocd()
        
        if pyocd then
            if pyocd.source == "package" then
                print("Using PyOCD from package: " .. pyocd.program)
            else
...
Make sure your debug probe is connected and drivers are installed.
        ...
        if option.get("address") then
            table.insert(argv, "--base-address")
            table.insert(argv, option.get("address"))
        end
